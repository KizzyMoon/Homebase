...
function decodeDocument(raw) {
  ...
  // Homebase has used two Firestore formats over its lifetime. Read BOTH and
  // merge them instead of ignoring the older structured dashboard whenever a
  // newer, possibly partial dashboardJson exists.
...
async function request(method, body) {
  ...
  try {
    let requestHeaders = await headers(false);
    ...
    if (response.status === 401) {
      requestHeaders = await headers(true);
...
function bad(error, where) {
  ...
  if (lastGood && failures < 3) {
    status("Synced · retrying", "busy", `${where}: ${error?.message || error}`);
  } else {
    status("Sync error", "error", `${where}: ${error?.message || error}`);
  }
}
...
async function saveNow() {
  ...
  try {
    ...
    // Do not let a stale second device overwrite a newer cloud copy. If cloud
    // advanced since this device last saw it, pull first and let the user edit
    // from the newest state.
    const cloud = await readCloud();
    const seen = Number(sessionStorage.getItem(VERSION_KEY) || 0);
    if (cloud && cloud.clientUpdatedAt > seen && cloud.updatedBy !== deviceId) {
      const changed = applyCloud(cloud.dashboard);
      sessionStorage.setItem(VERSION_KEY, String(cloud.clientUpdatedAt || 0));
      good("Pulled newer cloud data before save", cloud.dashboard);
...
async function poll() {
  ...
  try {
    const cloud = await readCloud();
    if (!cloud) {
      await writeCloud("Created missing cloud dashboard");
      return;
    }

    const seen = Number(sessionStorage.getItem(VERSION_KEY) || 0);
    const newerFromOtherDevice = cloud.clientUpdatedAt > seen && cloud.updatedBy !== deviceId;

    if (newerFromOtherDevice) {
      const changed = applyCloud(cloud.dashboard);
      sessionStorage.setItem(VERSION_KEY, String(cloud.clientUpdatedAt || 0));
      good("Pulled newer cloud data", cloud.dashboard);
      if (changed) setTimeout(() => location.reload(), 80);
    } else {
      good("Cloud read verified", cloud.dashboard);
    }
  } catch (error) {
    bad(error, "poll");
  }
...
onAuthStateChanged(auth, async (current) => {
  ...
  try {
    ...
    if (!cloud) {
      ...
      // Migrate partial/legacy documents into one complete v3 snapshot. This is
      // particularly important for calendarLinks, which could live only in the
      // old structured dashboard while dashboardJson contained other fields.
      const missingFields = KEYS.some((key) => !Object.prototype.hasOwnProperty.call(cloud.dashboard, fieldFor(key)));
      if (cloud.schemaVersion < 3 || missingFields) {
        await writeCloud("Migrated complete cloud dashboard");
      } else {
        good("Cloud dashboard loaded", cloud.dashboard);
      }
